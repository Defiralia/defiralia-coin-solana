...
pub fn valid_owner(state: &ContractState, signer: &AccountInfo) -> Result<()> {
    require!(signer.key.eq(&state.authority), DefiraliaCoinError::Unauthorized);
...
pub fn valid_signer(signer: &AccountInfo) -> Result<()> {
    require!(signer.is_signer, DefiraliaCoinError::Unauthorized);
...
pub fn ethereum_token_state_mapping_not_performed_yet(state: &ContractState) -> Result<()> {
    require!(
        ...
        DefiraliaCoinError::EthereumTokenStateMappingAlreadyPerformed
    );
...
pub fn parse_timestamp(timestamp: i64) -> Result<DateTime> {
    require!(timestamp >= 0, DefiraliaCoinError::InvalidTimestamp);
...
pub fn calculate_month_difference(start: i64, end: i64) -> Result<u64> {
    require!(end >= start, DefiraliaCoinError::EndTimeMustBeLaterThanStartTime);
    ...
    T: VestedWalletContext<'info>,
{
    require!(
        ...
        DefiraliaCoinError::NotEnoughTokens
    );
