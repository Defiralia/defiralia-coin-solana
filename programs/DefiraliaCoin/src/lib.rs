...
pub mod defiraliacoin {
    ...
    /// ### Arguments
    ///
    /// * `account_info_from_ethereum` - a set of accounts reflecting those used on Ethereum; DefiraliaCoin tokens are transferred to these accounts
    ...
    pub fn import_ethereum_token_state<'info>(
        ...
        for account in ctx.remaining_accounts.iter() {
            ...
            require!(
                ...
                DefiraliaCoinError::NonUniqueAccountInfo
            );

            let account_info = matching_accounts
                .first()
                .ok_or(DefiraliaCoinError::MismatchBetweenRemainingAccountsAndUserInfo)?;

            if wallet_names.contains(&account_info.wallet_name) {
                return Err(DefiraliaCoinError::DuplicatedWalletName.into());
            }
        ...
        require!(
            ...
            DefiraliaCoinError::ProgramAccountBalanceIsNotZero
        );
        require!(
            vesting_state.initial_community_wallet_balance != 0,
            DefiraliaCoinError::CommunityWalletBalanceIsZero
        );
        require!(
            vesting_state.initial_partnership_wallet_balance != 0,
            DefiraliaCoinError::PartnershipWalletBalanceIsZero
        );
        require!(
            vesting_state.initial_marketing_wallet_balance != 0,
            DefiraliaCoinError::MarketingWalletBalanceIsZero
        );
        require!(
            vesting_state.initial_liquidity_wallet_balance != 0,
            DefiraliaCoinError::LiquidityWalletBalanceIsZero
        );
    ...
    pub fn burn(ctx: Context<BurnContext>) -> Result<()> {
        ...
        require!(now.days <= 5, DefiraliaCoinError::TooLateToBurnTokens);
        require!(
            contract_state.last_burning_month != now.month
                || contract_state.last_burning_year != now.year,
            DefiraliaCoinError::TokensAlreadyBurned
        );
...
mod tests {
    ...
    async fn test_initialize() {
        let program_id = id();
        let program_test = ProgramTest::new("defiraliacoin", program_id, processor!(entry));
    ...
    async fn test_import_ethereum_token_state() {
        let program_id = id();
        let mut program_test = ProgramTest::new("defiraliacoin", program_id, processor!(entry));
